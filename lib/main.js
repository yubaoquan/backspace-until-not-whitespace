...
export default {
    ...
    registerListener() {
        ...
        this.disposer = this.buffer.onDidChangeText((e) => {
            if (this.isWhitespaceDeleted(e)) {
                ...
                this.deleteWhiteSpaces(false);
            }
        });
        this.element = atom.views.getView(editor);
        this.elementListener = this.storeLastPressedKey.bind(this);
        if (this.element.registered) {
            return;
        }
    ...
    registerCommand() {
        this.subscriptions.add(atom.commands.add('atom-text-editor:not([mini])', {
            'backspace-until-not-whitespace:delete-white-spaces': () => this.deleteWhiteSpaces(true),
        }));
    },
    deleteWhiteSpaces(fromCommand) {
        if (fromCommand && !config.useKeybinding) {
            return;
        };
    ...
    deactivate: function() {
        if (this.element) {
            this.element.removeEventListener('keydown', this.elementListener);
        }
        if (this.disposer) {
            this.disposer();
        }
